...
def multi_class_pose_decode(
    heat, cls_heat, wh, kps, reg=None, hm_hp=None, hp_offset=None, K=100
):
    batch, cat, height, width = heat.size()
    num_joints = kps.shape[1] // 2
    num_classes = cls_heat.shape[1]
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat = _nms(heat)
    scores, inds, _, ys, xs = _topk(heat, K=K)
    ...
    wh = _transpose_and_gather_feat(wh, inds)
    wh = wh.view(batch, K, 2)
    clses = _transpose_and_gather_feat(cls_heat, inds)
    clses = clses.view(batch, K, num_classes)
    clses = torch.max(clses, -1)[1].view(batch, K, 1)
    scores = scores.view(batch, K, 1)
