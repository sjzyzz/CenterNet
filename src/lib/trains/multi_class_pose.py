...
from models.losses import FocalLoss, RegL1Loss, RegLoss, RegWeightedL1Loss, BCELoss
from models.decode import multi_pose_decode, multi_class_pose_decode
...
class MultiClassPoseLoss(torch.nn.Module):
    def __init__(self, opt):
        super(MultiClassPoseLoss, self).__init__()
        self.crit = FocalLoss()
        self.cls_crit = BCELoss()
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
                ...
                if opt.dense_hp:
                    ...
                    output['hps'] = torch.from_numpy(
                        gen_oracle_map(
                            ...
                            batch['ind'].detach().cpu().numpy(), opt.output_res,
                            opt.output_res
                        )
                    ).to(opt.device)
            if opt.eval_oracle_hp_offset:
                output['hp_offset'] = torch.from_numpy(
                    gen_oracle_map(
                        ...
                        batch['hp_ind'].detach().cpu().numpy(), opt.output_res,
                        opt.output_res
                    )
                ).to(opt.device)
            ...
            if opt.dense_hp:
                ...
                hp_loss += (
                    self.crit_kp(
                        output['hps'] * batch['dense_hps_mask'],
                        batch['dense_hps'] * batch['dense_hps_mask']
                    ) / mask_weight
                ) / opt.num_stacks
            else:
                hp_loss += self.crit_kp(
                    output['hps'], batch['hps_mask'], batch['ind'], batch['hps']
                ) / opt.num_stacks
            if opt.wh_weight > 0:
            ...
            cls_loss += self.cls_crit(
                output['hm_cls'], batch['cls_mask'], batch['ind'], batch['cls']
            ) / opt.num_stacks
...
class MultiClassPoseTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(MultiClassPoseTrainer, self).__init__(opt, model, optimizer=optimizer)

    def _get_losses(self, opt):
        loss_states = [
            'loss', 'hm_loss', 'hp_loss', 'hm_hp_loss', 'hp_offset_loss', 'wh_loss',
            'off_loss', 'cls_loss'
        ]
        loss = MultiClassPoseLoss(opt)
            ...
            for k in range(len(dets[i])):
                if dets[i, k, 4] > opt.center_thresh:
                    debugger.add_coco_bbox(
                        dets[i, k, :4], dets[i, k, -1], dets[i, k, 4], img_id='out_pred'
                    )
    ...
    def save_result(self, output, batch, results):
        ...
        dets_out = multi_pose_post_process(
            dets.copy(), batch['meta']['c'].cpu().numpy(), batch['meta']['s'].cpu().numpy(),
