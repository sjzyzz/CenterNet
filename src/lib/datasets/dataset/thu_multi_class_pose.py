...
class THU_Multi_Class_Pose(data.Dataset):
    num_classes = 1
    real_num_classes = 3
    num_joints = 17
    ...
    def __init__(self, opt, split):
        super(THU_Multi_Class_Pose, self).__init__()
        self.edges = [
            [0, 1], [0, 2], [1, 3], [2, 4], [4, 6], [3, 5], [5, 6], [5, 7], [7, 9], [6, 8],
            [8, 10], [6, 12], [5, 11], [11, 12], [12, 14], [14, 16], [11, 13], [13, 15]
        ]
        ...
        print('==> initializing THU Basketball Human Pose {} data.'.format(split))
        ...
        for image_id in all_bboxes:
            for cls_ind in all_bboxes[image_id]:
                for dets in all_bboxes[image_id][cls_ind]:
                    ...
                    cls_cnt[int(category_id)] += 1
        for i in range(3):
            print(f'There are {cls_cnt[i]} instances of class {i}')
        return detections
