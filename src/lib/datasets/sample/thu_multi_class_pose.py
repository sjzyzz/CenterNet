...
class THUMultiClassPoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        inp = cv2.warpAffine(
            img,
            trans_input, (self.opt.input_res, self.opt.input_res),
        ...
        # 加入类别特征图
        # hm_cls = np.zeros((self.real_num_classes, output_res, output_res), dtype=np.float32)
        ...
        cls = np.zeros((self.max_objs, self.real_num_classes), dtype=np.float32)
        cls_mask = np.zeros((self.max_objs, self.real_num_classes), dtype=np.uint8)
