    ...
    for i in range(len(dets)):
        ...
        for cls_ind in dets[i].keys():
            ...
            for j in range(len(dets[i][cls_ind])):
                ...
                bbox = [
                    center[0] - wh[0] / 2, center[1] - wh[1] / 2, center[0] + wh[0] / 2,
                    center[1] + wh[1] / 2
                ]
...
def ctdet_post_process(dets, c, s, h, w, num_classes):
    ...
    for i in range(dets.shape[0]):
        ...
        for j in range(num_classes):
            ...
            top_preds[j + 1] = np.concatenate(
                [
                    dets[i, inds, :4].astype(np.float32), dets[i, inds,
...
def multi_class_pose_post_process(dets, c, s, h, w):
    # dets: batch x max_dets x 40
    # return list of 40 in image coord
    ret = []
    for i in range(dets.shape[0]):
        bbox = transform_preds(dets[i, :, :4].reshape(-1, 2), c[i], s[i], (w, h))
        pts = transform_preds(dets[i, :, 5:39].reshape(-1, 2), c[i], s[i], (w, h))
        # TODO cls这里的dets[-1:0]不知道是否合适
        top_preds = np.concatenate(
            [
                bbox.reshape(-1, 4), dets[i, :, 4:5],
                pts.reshape(-1, 34), dets[i, :, -1].reshape(-1, 1)
            ],
            axis=1
        ).astype(np.float32).tolist()
        ret.append({np.ones(1, dtype=np.int32)[0]: top_preds})
